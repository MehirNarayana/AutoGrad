...
Value::Value(double data, std::set<std::shared_ptr<Value>> children, Op op)
    : data(data), prev(children), op(op) {}

std::shared_ptr<Value> Value::operator+(const std::shared_ptr<Value>& other) {
    std::shared_ptr<Value> out =
        std::make_shared<Value>(this->data + other->getData(),
...
std::shared_ptr<Value> Value::operator+(double other) {
    ...
    std::shared_ptr<Value> out =
        std::make_shared<Value>(this->data + num->getData(),
                                std::set<std::shared_ptr<Value>>{shared_from_this(), num},
                                Op::Add);
    auto currBackward = [=]() {
        shared_from_this()->gradient += 1 * out->gradient;
...
std::shared_ptr<Value> Value::operator^(double exponent) {
    ...
    std::shared_ptr<Value> out =
        std::make_shared<Value>(std::pow(this->data, exponent),
                                ...
                                Op::Pow);
    auto currBackward = [=]() {
        shared_from_this()->gradient +=
            exponent * std::pow(this->data, exponent - 1) * out->gradient;
    };
...
std::shared_ptr<Value> Value::tanh() {
    ...
    double t = std::tanh(x);
    std::shared_ptr<Value> out =
        std::make_shared<Value>(t, std::set<std::shared_ptr<Value>>{shared_from_this()});
